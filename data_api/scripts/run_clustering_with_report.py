...
def main():
    parser = argparse.ArgumentParser(description='クラスタリング実行 + HTMLレポート自動表示')
    ...
    parser.add_argument('--size-min', type=int, default=10, help='最小クラスタサイズ (default: 10)')
    parser.add_argument('--size-max', type=int, default=50, help='最大クラスタサイズ (default: 50)')
    ...
    cmd = [
        'uv', 'run', 'python', 'message_clustering.py',
    ...
    # HTMLレポートを開く
    if not args.no_open:
        html_path = Path("output/message_clustering/clustering_report.html")
        if html_path.exists():
            print("\n" + "=" * 60)
