...
import { Play, Pause, Square, Clock, Edit3, X, Timer } from "lucide-react"
...
interface WorkSessionExtensionProps {
  messageId: string
  data: WorkSessionExtensionData
  onUpdate: (data: WorkSessionExtensionData) => void
  onDelete: () => void
}

export function WorkSessionExtension({
  messageId,
  data,
  onUpdate,
  onDelete
}: WorkSessionExtensionProps) {
  ...
  const handleCheckOut = () => {
    ...
    try {
      ...
      const timeSpent = data.checkedInAt
        ? Math.round((Date.now() - new Date(data.checkedInAt).getTime()) / (1000 * 60))
        : 0
